...
(function () {
        ...
        if (this._isOpen) {
            ...
            switch (drawingType) {
                ...
                case BMAP_DRAWING_RECTANGLE:
                    this._bindRectangle();
                    break;
                case PJ_ADMINISTRATIVE_AREA:
                    this._bindAdministrativeArea();
                    break;
            }
        }
     ...
     * 绑定画行政区域的事件
     */
    DrawingManager.prototype._bindAdministrativeArea = function () {
        ...
        var chooseMarker;
        var polygons = [];
        var areaLevelChooseControl;

        /**
         * 设置当前显示的区域
         */
        var setViewArea = function (point) {
            geoc.getLocation(point, function (rs) {
                var addComp = rs.addressComponents;
                console.log(addComp.province + ", " + addComp.city + ", " + addComp.district + ", " + addComp.street + ", " + addComp.streetNumber);
                var address = "";
                switch (areaLevelChooseControl.getSelectedType()) {
                    case 0://省
                ...
                console.debug("正在准备获取区域：" + address + "的行政区划信息");
                if (address == "")
                    return;
                if (lastMarkAddress == address)
                    return;
                lastMarkAddress = address;
                bdary.get(lastMarkAddress, function (rs) {       //获取行政区域
                    ...
                    if (count === 0) {
                        ...
                        console.warn('为行政区' + address + "找到了" + count + "个行政区域");
                    chooseMarker.setTitle(lastMarkAddress);
        ...
        var startAction = function (e) {
            ...
            if (!isBinded) {
                isBinded = true;
                chooseMarker = new BMap.Marker(e.point, {
                    icon: new BMap.Icon('css/flag.png', new BMap.Size(32, 32)),
                    offset: new BMap.Size(10, -14)
                });
                //chooseMarker = new BMap.Marker(e.point);//, { icon: new BMap.Icon('Scripts/maplib/flag.png', new BMap.Size(19, 25)) });
                map.addOverlay(chooseMarker);
                setViewArea(e.point);
                chooseMarker.enableDragging();
                chooseMarker.setZIndex(900);
                mask.hide();
                chooseMarker.addEventListener("dragend", function (ee) {
    ...
    DrawingManager.prototype._dispatchOverlayComplete = function (overlay, calculate) {
        ...
        if (this._opts.resetToolBar) {
            this._drawingTool.setStyleByDrawingMode("hander");
            this._drawingTool._bindEventByDraingMode("hander");
        }
    }
        ...
        baidu.on(this.panel, 'click', function (e) {
            ...
            if (me.drawingToolOptions.beforeDraw) {
                if (drawingType == "hander")
                    return;
                if (!me.drawingToolOptions.beforeDraw()) {
                    me.setStyleByDrawingMode("hander");
                    me._bindEventByDraingMode("hander");
                    return;
                }
            }
    ...
    function AreaLevelChooseControl() {
        // 默认停靠位置和偏移量
    ...
    AreaLevelChooseControl.prototype = new BMap.Control();
    AreaLevelChooseControl.prototype.initialize = function (map) {
        var me = this;
        var createItem = function (cls, text, type) {
            ...
            item.onclick = function () {
                ...
                if (t == "-1") {
                    if (me._finishCallback) {
                        me._finishCallback();
                    }
                    return;
                }
                var items = document.getElementsByClassName("groupItem");
                for (var i = 0; i < items.length; i++) {
                    items[i].className = items[i].className.replace("groupItem-checked", "");
                }
                item.className += " groupItem-checked";
                if (t) {
                    me._selectedType = t;
                }
            };
        ...
        div.appendChild(createItem("groupItem", "市", 1));
        div.appendChild(createItem("groupItem groupItem-checked", "县", 2));
        var okdiv = createItem("groupItem groupItem-right groupItem-ok", "", -1);
        okdiv.setAttribute("title", "完成");
        div.appendChild(okdiv);
        // 设置样式
    ...
    AreaLevelChooseControl.prototype.getSelectedType = function () {
        return parseInt(this._selectedType);
    }
...
function ElectronicFence(map, opts) {
    ...
    var me = this;
    var pushOverlay = function (overlay) {
        if (!me._allowMulti) {
            me._ef = new Array();
        }
        me._ef.push(overlay);
                ...
                if (me._mainOverlays.length != 0) {
                    if (confirm("当前已经绘制了电子围栏是否删除继续绘制新的？")) {
                        for (var i = 0; i < me._mainOverlays.length; i++) {
                            map.removeOverlay(me._mainOverlays[i]);
                        }
                        return true;
                    }
                    else {
...
ElectronicFence.prototype.setElectronicFence = function (efs) {
    ...
    var me = this;
    var drawcircle = function (ef) {
        var circle = new BMap.Circle(ef.center, ef.radius); //创建圆
        me._map.addOverlay(circle);
        me._mainOverlays.push(circle);
    }
    var drawpolygon = function (ef) {
        var ps = new Array();
        for (var i = 0; i < ef.points.length; i++) {
            ps.push(new BMap.Point(ef.points[i].lng, ef.points[i].lat));
        }
        var polygon = new BMap.Polygon(ps);
        me._map.addOverlay(polygon);
        me._mainOverlays.push(polygon);
    }
    var drawrectangle = function (ef) {
        var pStart = ef.ne;
        var pEnd = ef.sw;
        ...
        me._map.addOverlay(rectangle);
        me._mainOverlays.push(rectangle);
    }
    var drawadministrativearea = function (ef) {
        var bdary = new BMap.Boundary();

        bdary.get(ef.text, function (rs) {       //获取行政区域
            var count = rs.boundaries.length; //行政区域的点有多少个
            if (count === 0) {
                console.warn('未能获取当前输入行政区域');
                return;
            ...
            for (var i = 0; i < count; i++) {
                var ply = new BMap.Polygon(rs.boundaries[i], { strokeWeight: 2, strokeColor: "#ff0000" }); //建立多边形覆盖物
                me._map.addOverlay(ply);  //添加覆盖物
                me._mainOverlays.push(ply);
    ...
    for (var i = 0; i < this._ef.length; i++) {
        var ef = this._ef[i];
        switch (ef.type) {
            case "circle"://圆
                drawcircle(ef);
